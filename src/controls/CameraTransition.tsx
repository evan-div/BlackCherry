...
import { CAMERA_TRANSITION_SECONDS, DEFAULT_CAMERA, DEFAULT_WALK } from '../config/defaults';
...
export function CameraTransition() {
  ...
  useEffect(() => {
    ...
    if (mode === 'walk') {
      ...
      goalPos.current.set(camera.position.x, groundY + DEFAULT_WALK.eyeHeight, camera.position.z);
      ...
      goalPos.current.set(...DEFAULT_CAMERA.position);
      _lookMatrix.lookAt(goalPos.current, new Vector3(...DEFAULT_CAMERA.target), camera.up);
